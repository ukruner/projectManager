...
export default function NewProject({
  ...
  function handleSave() {
    ...
    findCurrentVal = projects.filter((entry) => {
      return entry.id === combination;
    });
    if (
      titleRef.current.value &&
      descRef.current.value &&
      dueDateRef.current.input.value
    ) {
  ...
  return (
    ...
    >
      <h2 className="mb-8 font-bold uppercase md:text-xl text-stone-800">
      ...
      <div className="flex items-center gap-4">
        <Input ref={titleRef} classic={true}>
          Title
        </Input>
        <Input ref={descRef} textarea={true}>
          Description
        </Input>
        <Input ref={dueDateRef} datearea={true}>
          Due Date
        </Input>
      </div>
      {output && (
